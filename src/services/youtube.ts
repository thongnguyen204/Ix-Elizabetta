...
export class YoutubeService {
    public static async getVideoDetails(content: string): Promise<Song> {
        const parseContent = content.match(youtubeVideoRegex);
        let id = '';
        if (!parseContent) {
    ...
    private static async searchVideo(search: string): Promise<string> {
        const result = await ytsr(search, { pages: 1 });
        const filteredResult = result.items.filter((item) => item.type === 'video');
        if (filteredResult.length === 0) throw new Error();
        const item = filteredResult[0] as Video;
